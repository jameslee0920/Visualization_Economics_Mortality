---
title: "Study on Economic <br> Conditions Across <br> Different Income Regions"
author: "James Lee"
...
gbcapY1 = ggplot(data = Table3, aes(x = log(Avg2000), y = Both.sexes..birth.2000)) + 
  geom_point(aes(color = IncomeGroup, alpha = "10" )) + 
  geom_point(data = Table3, aes(log(x = Avg2012), y = Both.sexes..birth.2012, color = IncomeGroup, alpha = "1"))  +
  ylab("Average Life Expectancy") +
  xlab("Log Average GDP per Capita") +
  ggtitle("Life Expectancy v. GDP per Capita \n Year 2000") +
...
gbcapY2 = ggplot(data = Table3, aes(x = log(Avg2000), y = Both.sexes..birth.2000)) + 
  geom_point(aes(color = IncomeGroup, alpha = "1" )) + 
  geom_point(data = Table3, aes(log(x = Avg2012), y = Both.sexes..birth.2012, color = IncomeGroup, alpha = "10"))  +
  ylab("Average Life Expectancy") +
  xlab("Log Average GDP per Capita") +
  ggtitle("Life Expectancy v. GDP per Capita \n Year 2012") +
  labs(color = "Income Group") +
...
- This project is designed to visualize the effects of the Economic Status of a State on the Life and Death of its Residents from year 2000 to year 2012
- Primarily, the Average Life Expectancy and the GDP per Capita of both Sex are compared to show the conditions of Countries in comparison with each other. 

#How does the world look?
## Aggregated on Average
```{r}
gg

```
- There is an apparent correlation between the Life Expectancy and the GDP per Capita

##Highlighted on Region
```{r}
Rbc
```
...
##Highlighted on Income Groups
```{r}
Ibc
```

##How do Income Gaps Differ?
```{r}
grid.arrange(Ibar, Ibar2, nrow=2)

```

##How do Causes of Mortality Differ?
```{r}
gLSub2012

```

##Cause of Mortality 2012 v. 2000
```{r}
gLSub2000
```
...
##Healthier People, More Money Earned
```{r}
grid.arrange(gbcapY1, gbcapY2, ncol=2)
```
<br>- Medical Advances allowed people to live longer <br>
- Longer Life Expectancy meant people are more willing to <br>invest in human capital (knowledge + experience)

#Change within Income Groups
...
- High Income Groups Tend to have lower GINI coefficients and hence lower income gaps
- Primary Cause of Mortality in Low Income Groups are Communicable Diseases
- Over time, the economic conditions of countries grow better (significant in Low Income Regions)
- As countries grow economically, Cancer and Cardiovascular Diseases rise significantly and then decrease

##Next Steps
- Increase the range of time in years
- Analyze mortality rate on different age groups
- Analyze Economic Conditions by Sex
- Analyze Economic Conditions by Population Density (City, Rural Areas)
- Big Mac Index
